...
public class Crossword {
	...
	private final char[] board; // Current board state. _ is unfilled. # is blocked. other characters are
								// filled.
	private final Set<String> words; // List of words
	private final Map<Point, Integer> vertical = new HashMap<>(), horizontal = new HashMap<>(); // Vertical and
																								// horizontal slots
	...
	private Crossword(List<String> lines) {
		...
		log("A " + width + "x" + height + " board, " + vertical.size() + " vertical, " + horizontal.size()
				+ " horizontal.");
	...
	// Recur each slot, try every word in a loop. When all slots of this kind are
	// filled successfully, run next stage.
	private boolean solve(Map<Point, Integer> slot, BiFunction<Point, String, Boolean> fill, String dir,
			Supplier<Boolean> next) {
		if (slot.isEmpty())
